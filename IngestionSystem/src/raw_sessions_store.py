...
class RawSessionsStore:
    ...
    def __init__(self) -> None:
        ...
        db_path = os.path.join(os.path.abspath('..'), 'data', self.config['db_path'])
    ...
    def open_connection(self) -> bool:
        ...
        """
        try:
            self._conn = sqlite3.connect(os.path.join(os.path.abspath('..'), 'data', self.config['db_path']))
            return True
    ...
    def load_raw_session(self, uuid: str) -> dict:
        ...
        try:
            ...
            if result[2] is not None:
                raw_session['pressure_detected_label'] = json.loads(result[2])['label']
            ...
            for time_series in result[4:]:
                if time_series is not None:
                    ...
                    raw_session['time_series'].append(time_series_data[3:])
                else:
        ...
        try:
            if last_missing_sample:
                ...
                query = 'SELECT COUNT(1) FROM raw_session WHERE uuid = ? ' \
                        ...
                        + ('AND label IS NOT NULL' if (operative_mode == 'development' or evaluation) else '')
            else:
